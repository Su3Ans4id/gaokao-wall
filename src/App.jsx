...
import { fetchWishes, createWish, subscribeToWishes } from './lib/supabase'
...
function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="relative overflow-hidden bg-gradient-to-br from-gaokao-red via-red-600 to-red-700 text-white">
        ...
        <div className="relative px-4 py-10 md:py-16 text-center">
          <div className="inline-flex items-center gap-2 mb-3">
            <span className="text-gaokao-gold text-3xl md:text-4xl">&#x2728;</span>
            <h1 className="text-3xl md:text-5xl font-extrabold tracking-wide">高考加油</h1>
            <span className="text-gaokao-gold text-3xl md:text-4xl">&#x2728;</span>
          </div>
          <p className="text-red-100 text-sm md:text-lg max-w-md mx-auto leading-relaxed">
            每一份祝福，都是前行的力量<br/>
      ...
      <main className="max-w-6xl mx-auto px-3 md:px-4 py-6">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <button
              onClick={loadWishes}
              className="px-6 py-2 bg-gaokao-red text-white rounded-full text-sm hover:bg-red-700 transition-colors"
            >
              重新加载
            </button>
          </div>
      ...
      {/* 表单弹窗 */}
      {showForm && (
        <WishForm onSubmit={handleSubmit} onClose={() => setShowForm(false)} />
      )}
